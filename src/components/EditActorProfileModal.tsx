...
interface EditActorProfileModalProps {
  isOpen: boolean;
  onClose: () => void;
  actor: {
    id: number;
  ...
  onUpdate: (userId: number, data: { name: string; email: string; phone?: string }) => Promise<void>;
  isPending: boolean;
}

export function EditActorProfileModal({
  isOpen,
  onClose,
  actor,
  onUpdate,
  isPending,
}: EditActorProfileModalProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
