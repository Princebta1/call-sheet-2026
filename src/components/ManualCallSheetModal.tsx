...
import { X, FileText, Calendar, MapPin, Plus, Trash2 } from "lucide-react";
import toast from "react-hot-toast";

interface ManualCallSheetModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedDate?: Date | null;
}

export function ManualCallSheetModal({
  isOpen,
  onClose,
  selectedDate,
}: ManualCallSheetModalProps) {
  const trpc = useTRPC();
  ...
  const createCallSheetMutation = useMutation(
    trpc.createManualCallSheet.mutationOptions({
      onSuccess: () => {
      ...
      onError: (error) => {
        toast.error(error.message || "Failed to create call sheet");
      },
    })
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const filteredScenes = scenes.filter(s => s.trim() !== "");
    const filteredCrew = crewMembers.filter(c => c.trim() !== "");
    const filteredActors = actors.filter(a => a.trim() !== "");
    ...
    createCallSheetMutation.mutate({
      ...
      scenesIncluded: JSON.stringify(filteredScenes),
      crewList: JSON.stringify(filteredCrew),
      actorList: JSON.stringify(filteredActors),
      notes: notes || undefined,
    });
  };

  const handleClose = () => {
    if (!createCallSheetMutation.isPending) {
      setShowId(null);
      setDate("");
      setLocation("");
      setNotes("");
      ...
      <Dialog as="div" className="relative z-50" onClose={handleClose}>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 to-gray-900/95 border border-gray-800 p-6 shadow-2xl transition-all max-h-[90vh] overflow-y-auto">
                ...
                {shows.length === 0 ? (
                  <div className="space-y-4">
                    <div className="p-4 bg-cinematic-blue-500/10 border border-cinematic-blue-500/30 rounded-lg">
                      <p className="text-cinematic-blue-400 text-sm mb-3">
                        You need to create a show before creating a call sheet.
                      </p>
                      <Link
                        to="/shows/new"
                        onClick={handleClose}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          disabled={createCallSheetMutation.isPending}
                          className="text-sm text-cinematic-gold-400 hover:text-cinematic-gold-300 flex items-center gap-1 disabled:opacity-50"
                        >
                        ...
                        {scenes.map((scene, index) => (
                          <div key={index} className="flex gap-2">
                            <input
                              ...
                              placeholder="e.g., Scene 1 - Interior Kitchen"
                              disabled={createCallSheetMutation.isPending}
                              className="flex-1 px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-cinematic-gold-500 focus:border-transparent transition-all disabled:opacity-50"
                            />
                            {scenes.length > 1 && (
                              <button
                                type="button"
                        ...
                        {actors.map((actor, index) => (
                          <div key={index} className="flex gap-2">
                            <input
                              ...
                              placeholder="e.g., Jane Smith - Lead Role"
                              disabled={createCallSheetMutation.isPending}
                              className="flex-1 px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-cinematic-gold-500 focus:border-transparent transition-all disabled:opacity-50"
                            />
                            {actors.length > 1 && (
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button
                        type="submit"
                        disabled={createCallSheetMutation.isPending || !showId || !date}
                      ...
                      >
                        {createCallSheetMutation.isPending ? "Creating..." : "Create"}
                      </button>
                    </div>
                  </form>
                )}
